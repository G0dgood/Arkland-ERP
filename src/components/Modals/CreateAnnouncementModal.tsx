...
const CreateAnnouncementModal = () => {
  ...
  const availablleRoles = roles?.map((role: any) => ({
    value: role.id,
    label: role.name,
  })) || [];
  const availablleTeams = teams?.map((team: any) => ({
    value: team.id,
    label: team.name,
  })) || [];
  const availablleDepartments = departments?.map((department: any) => ({
    value: department.id,
    label: department.name,
  })) || [];
  const availablleProject = projects?.map((project: any) => ({
    value: project.id,
    label: project.name,
  })) || [];
