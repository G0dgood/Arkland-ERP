    ...
    >
      ...
      <SidebarContent>
        <Menu iconShape="circle">
          <MenuItem className='Side__Content' active={pathname === '/dashboard'} icon={<TfiLayoutGrid2 size={17} />} suffix={<span className="badge red">NEW</span>} >
      ...
      Application  <Link to="/application" /> </MenuItem> */}
          ...
          {(isTeamLead || isHRHead || isSuperAdmin || isAdmin || isHrAdmin || isMaster || isHeadOfDepartment) && (
            <SubMenu title={'Pay Roll'} icon={<FaRegAddressCard size={17} />}>
              {(isSuperAdmin || isHRHead || isHrAdmin) && (
                <MenuItem className='Side__Content' active={pathname === '/payroll/payroll/payrollemployees'}>
                  <Link to="/payroll/payroll/payrollemployees" />Employees</MenuItem>
              )}
              {(isSuperAdmin || isHRHead || isHrAdmin) && (
                <MenuItem className='Side__Content' active={pathname === '/payroll/payroll/payparameters'}>
                  <Link to="/payroll/payroll/payparameters" />Pay Parameters</MenuItem>
              )}
              {(isSuperAdmin || isTeamLead || isHeadOfDepartment) && (
                <MenuItem className='Side__Content' active={pathname === '/payroll/payroll/payrun'}> <Link to="/payroll/payroll/payrun" />Pay Run</MenuItem>
              )}
              {(isSuperAdmin || isAdmin || isHRHead) && (
                <MenuItem className='Side__Content' active={pathname === '/payroll/payroll/approvalrequests'}>  <Link to="/payroll/payroll/approvalrequests" />Approved Requests</MenuItem>
              )}
              {(isSuperAdmin || isAdmin || isHRHead) && (
                <MenuItem className='Side__Content' active={pathname === '/payroll/payroll/salaryincrement'}>  <Link to="/payroll/payroll/salaryincrement" />Salary Increment</MenuItem>
              )}
            </SubMenu>)}
        </Menu>
      </SidebarContent>
