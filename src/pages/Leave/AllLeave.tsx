...
const AllLeave = () => {
	...
	return (
		...
		<div id="reports">
			<h5 className="page-title">Employee Leave Applications</h5>
			<div className='half-background mt-4'>
				<SearchComponent sortData={sortData} entriesPerPage={entriesPerPage} setEntriesPerPage={setEntriesPerPage} placeholder={"Leave Applications"} />
										...
										<tr className="data-table-row" key={i}>
											...
											<td className="table-datacell datatype-numeric">
												<Link to={`/leave/leave/final/${item?._id}`}  >
													{/* @ts-ignore */}
													{item?.status === "rejected" ? "" :
														<Button id="view-status">{item?.finally_approved === false ? "Update" : "View"}</Button>}
												</Link>
											</td>
										</tr>
									))
