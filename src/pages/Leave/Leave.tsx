...
const Leave = () => {
 ...
 return (
  ...
  <div id="reports">
   <h5 className="page-title">Create Leave</h5>
   <ApplyForLeave />
   <div className='half-background mt-4'>
    <SearchComponent sortData={allLeavedata?.data} entriesPerPage={entriesPerPage} setEntriesPerPage={setEntriesPerPage} placeholder={"Create Leave"} />
          ...
          <tr className="data-table-row" key={i}>
           ...
           <td className="table-datacell datatype-numeric">
            <Link to={`/leave/leave/${item?._id}`}  >
             {item?.status === "rejected" ? "" :
              <Button id="view-status">{item?.hod_approved === false ? "Update" : "View"}</Button>}
            </Link>
           </td>
          </tr>
         )))}
